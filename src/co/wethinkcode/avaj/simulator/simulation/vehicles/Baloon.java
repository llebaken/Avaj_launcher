    ...
    public void updateConditions(){
        ...
        switch (weather){
            ...
            case "RAIN":
                ...
                Simulator.writer.println("Baloon#"+this.name+"("+this.id+")"+" It's raining. Better watch out for lightings");
                break;
            case "FOG":
                this.coordinates = new Coordinates(this.coordinates.getLongitude(), this.coordinates.getLatitude(), this.coordinates.getHeight() - 3);
                Simulator.writer.println("Baloon#"+this.name+"("+this.id+")"+" I can't see through this fog");
                break;
            case "SNOW":
                this.coordinates = new Coordinates(this.coordinates.getLongitude(), this.coordinates.getLatitude(), this.coordinates.getHeight() - 15);
                Simulator.writer.println("Baloon#"+this.name+"("+this.id+")"+" Winter is coming");
                break;
        }
        if (this.coordinates.getHeight() <= 0){
            this.weatherTower.unregister(this);
            Simulator.writer.println("Tower says: Baloon#"+this.name+"("+this.id+")"+" unregistered to the weather tower");
        }
    }
    public void registerTower(WeatherTower weatherTower){
        this.weatherTower = weatherTower;
        this.weatherTower.register(this);
        Simulator.writer.println("Tower says: Baloon#"+this.name+"("+this.id+")"+" registered to the weather tower");
    }
}
